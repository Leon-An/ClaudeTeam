...
def _host_cpu(
    ...
    *,
    ...
    if loadavg:
        ...
        return {"load": (l1, l5, l15), "cores": ncores, "pct": int(round(l1 / max(ncores, 1) * 100))}
    # macOS host fallback — no /proc, but `uptime` is on PATH.
    r = run(["uptime"])
    if r.returncode != 0:
        return None
    m = re.search(r"load average[s]?:\s*([\d.]+)[,\s]+([\d.]+)[,\s]+([\d.]+)", r.stdout)
    if not m:
        return None
...
def _host_mem(run: Callable = _run, *, read_proc: Callable[[str], str | None] = _read_proc) -> dict | None:
...
def _docker_stats(run: Callable = _run) -> list[dict]:
    ...
    r = run(["sudo", "-n", "docker", "stats", "--no-stream", "--format", "{{json .}}"], timeout=15)
    if r.returncode != 0:
        return []
    status_r = run(["sudo", "-n", "docker", "ps", "--format", "{{.Names}}\t{{.Status}}"])
...
def _subtree_usage(root_pid: int, procs: dict, children: dict) -> tuple[float, int]:
...
def _agent_usage(agent_set: frozenset, session: str, run: Callable = _run) -> list[dict]:
    ...
    """
    r = run(["tmux", "list-panes", "-a", "-F", "#{session_name}:#{window_name} #{pane_pid}"])
    if r.returncode != 0:
        return []
    panes: dict[str, int] = {}
    for line in r.stdout.splitlines():
    ...
    for agent, pid in panes.items():
        ...
        out.append({"agent": agent, "location": session, "cpu": cpu, "mem": mem})
    return out
...
def _collect_alarms(
    host_mem: dict | None, host_disk: dict | None, containers: list[dict], run: Callable = _run
) -> list[str]:
    ...
    if host_mem and host_mem["pct"] >= 90:
        alarms.append(f"主机内存 **{host_mem['pct']}%**（used {fmt_bytes(host_mem['used'])}）")
    ...
    if dm.returncode == 0 and dm.stdout:
        oom = [line for line in dm.stdout.splitlines()[-500:] if re.search(r"out of memory|killed process", line, re.I)]
        if oom:
...
def collect_server_load(agent_set: frozenset, session: str, run: Callable = _run) -> dict:
