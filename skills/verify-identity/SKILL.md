...
适用角色: 部署 agent / manager（巡视时可对单个员工复用判读部分）
触发: 部署 bootstrap 注入全员身份之后（必跑）；巡视中怀疑某员工身份状态时（单点复用）
预计耗时: 每员工 1-3 分钟（含等待），全队一轮 5-15 分钟
---

# verify-identity — 逐员工确认身份消息已被正确处理

## 何时用

部署流程的最后一道闸：团队拉起 → 每个员工的身份 init 消息注入完成 → **执行本 skill** → 全员通过才允许宣布部署成功。`claudeteam health` 绿不算数——health 只看进程不看"人"。巡视中怀疑某员工没在状态时，可单独对它执行第二步起的判读流程。
...
1. **给足处理时间**。身份消息注入后，员工需要时间读身份文件、查收件箱、回报到位。先等一两分钟量级（慢模型更久）再开始确认——注入完立刻看，看到的多半是还没开始的画面，会误判。

2. **逐员工拉取整段现场，自己读**。对每一个员工（一个都不能跳过），把 pane 全文拉出来，像读一份终端实录一样从头到尾读完，回答一个问题：

   > **这个员工是否已经以正确的身份，完成了身份消息要求它做的事，并处于能接活的状态？**
